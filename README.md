![](https://git.halogenos.org/halogenOS/android_manifest/-/raw/XOS-15.1/halogenos-logo.png)
...
To get started with XOS, you should first become familiar with the basics of the utilities named [Git](http://rogerdudler.github.io/git-guide/) and [repo](https://source.android.com/source/using-repo.html). Please install `repo` and `git` according to the AOSP documentation before proceeding.

We provide everything needed to build in a `shell.nix` file. To make sure you have everything necessary installed to build XOS, please install the Nix package manager and use `nix-shell` to drop into a shell that has everything you need. You can also use Arch Linux or any other distribution, however, in such a case it is up to you to find out what you need.
...
```bash
repo init -u https://git.halogenos.org/halogenOS/android_manifest.git -b XOS-15.1
```
...
If you're using the nix shell we provide, all commands going forward must be run in said shell. Use the `nix-shell` command in the top level directory to enter it.
...
```bash
build full aosp_<device>-ap4a-userdebug
```
